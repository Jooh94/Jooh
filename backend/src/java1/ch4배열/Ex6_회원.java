	...
	public static void main(String[] args) { // main s
		...
		while( true ) { // while start [ 종료조건 : 없다 ]
			...
			if( btn == 1 )   // 1 입력했을떄
				...
				System.out.println("================= 로그인 페이지 ===============");
				...
				else if(btn ==2)// 2입력했을때
					{ 
					System.out.println("================회원가입 페이지==============");
							//1. 회원정보 4개의 변수로 입력받기 [바로 배열에 대입X] 
					System.out.println("아이디 입력해주세요:"); String id =scanner.next(); 
					...
					System.out.println("비밀번호 입력해주세요:");String password =scanner.next();
					System.out.println("이름 입력해주세요:"); String name =scanner.next();
					System.out.println("전화번호 입력해주세요:");String phone = scanner.next();
					 ...
					 //3. 입력받은 배열에 저장한다. [배열호출 : 배열명[행인덱스][열인덱스]
					 		...
					 		for ( int row =0 ;  row<memberlist.length ; row++) {
					 			...
					 			if ( row == 99) { System.out.println("안내) 최대 회원수가 찼습니다. [회원가입불가능:관리자에게 문의]");}
					 		}// for e
					} //if 3 end
					
					}// else if end
				
				else if(btn ==3)// 3입력했을때
					{
					System.out.println("=====================아이디찾기===================");
						...
						for( int i = 0 ; i<memberlist.length ; i++) {
							...
							if ( i == memberlist.length-1) { // i 가마지막 인덱스와 같으면 [모두 검사했는데 조건못찾음]
								System.out.println("안내)동일한 회원정보가 없습니다.");
							}// if end
						}
						//3. 존재하면 해당 아이디 출력 아니면 미출력
					} // for end
				else if(btn ==4)// 4입력했을때
					{ 
						...
						for ( int i =0 ; i<memberlist.length ; i ++) {
							...
							if( i == memberlist.length-1) {
								System.out.println("안내) 동일한회원정보가 없습니다.");
						}
					}
  	...
  	회원제 프로그램 구현
  		...
  		[기능]
  			1. 회원가입 기능
  				...
  				3.아이디 중복체크
  				4.최대 회원 100명
  					*1명 4개 변수 -> 400변수 -> 2차원 배열
					* 행 당 1명
					* 0열 : 아이디 1열:비밀번호 2:이름 3:번화번호
  			2. 로그인 기능
