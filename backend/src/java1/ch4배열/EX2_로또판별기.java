...
/* 
 	로또 판별기
 		1. 구매번호 :사용자 에게 6개 수 입력 -> 6변수X -> 배열저장
...
public class EX2_로또판별기 {// class s
	...
	public static void main(String[] args) { // main s
		...
		//1. 사용자에게 입력받는다. [6번]/////////////////////////////////////////////////////////
		for ( int i = 0 ; i<6 ; i++) { // i = 인덱스 // // isms 0부터 5까지 1씩 증가
			...
			if(구매번호[i] < 1 || 구매번호[i] >45) { //만약에 입력값이 1보다 작거나 45보다 크면 재입력
				System.err.println("안내) 선택할수 없는 번호 입니다.");
				i--; //현재 i 무효처리 
			...
			for( int j = 0 ; j<i ; j++) {
				if ( 구매번호[j] == 구매번호[i]) { // 만약에 j 번째 인덱스의 값 i번째 인덱스와 같으면
