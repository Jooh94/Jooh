	...
	* 학생 점수 프로그램
		...
		2. HTMl에서 입력받은 데이터로 js객체에 저장
			* 학생 이름 중복체크 함수
			...
			4. HTML 초기화
		3. 학생 점수출력
			1.js 처음 열렸을때 1번 / 2. 학생 등록할때마다
			
*/
//
let studentlist = [ ] // *여러개 학생 객체를 저장하는 배열 [ 함수 밖에 선언한 이유 : 여러 함수플에서 사용하기 위해서]
학생출력()

function 점수등록 () { //2. 점수 등록 버튼을 눌렀을때
	//1.학생 객체 : HTML 입력받은 데이터로 선언
		// document.getElementById('kor').value : input 에 입력된 무조건 문자열 가져오기
		// Number (document.getElementById('kor').value ) :input에 입력된 문자열을 숫자열에 변환해서 가져오기
const student = {
	...
	if(이름중복체크(student.name) ) {
		return // return 함수 종료 하는 의미로 return 될 경우에는 아래 코드는 실행되지 않는다.
	}
...
function 학생출력() {
	...
	for( let i = 0 ; i<studentlist.length ; i++){
		// i는 0부터 배열내 마지막인덱스까지 [배열의길이-1] 1씩증가반복
