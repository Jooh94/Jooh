...
let mpassword = document.querySelector('#mpassword').value // doGet 으로이동
let loginconfirmbox= document.querySelector('#loginconfirmbox')
//2.변수 2개를 서블릿으로 보내자[JQUERY :자바스크립트 라이브러리 ]
		//1. JQEURY 라이브러리 포함	//2. AJAX : (비동기식 통신기법)
	//$.ajax({속성명 = 값, 속성명 = 값})
	// url : '서블릿URL'
	...
	$.ajax({
		...
		success: function(re){
			...
			else if (re ==='2'){loginconfirmbox.innerHTML = '회원정보가 다릅니다'}
			else if(re==='3'){loginconfirmbox.innerHTML = '데이터베이스 오류[관리자문의]'}
			else if(re==='0'){loginconfirmbox.innerHTML = '존재하지않는 아이디 입니다'}
		}
	});
