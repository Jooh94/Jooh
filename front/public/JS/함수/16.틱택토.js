...
function 게임결과 () {
    ...
    for( let i = 0 ; i<=6 ; i+=3){
		...
		if( 게임상황[i] !=null && 게임상항[i] == 게임상황[i+1] && 게임상황[i+1] == 게임상황[i+2] ){
		// && 이면서 면서 이고 모두 그리고
		// i가 0 일때 0 == 1 ==2 //i가 3일때 3 == 4 == 5 // i가 6일때 6 == 7== 8
	...
	for( let i = 0 ; i<=2 ; i++){
		...
		if( 게임상황[i] !=null && 게임상황[i] == 게이망황[i+3] && 게임상황[i+3] == 게임상황[i+6] ){
			승리 = 게임상황[i] // 승리 변수에 i번째 알 저장
			return true // 가로로 승리가 나왔을때 경우 함수 종료
		}
	}
	// 3. 대각선으로 이기는 수 [ 0 4 8 , 2 4 6]
	// 4. 무승부 [ 위 상황이 아니면서 알이9개 이면]
