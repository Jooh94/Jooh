...
public class JDBC실행 {
	...
	public static void main(String[] args) {
		...
		// 2. 해당 프로젝트 라이브러리 추가
			...
			// 5. apply
		// 3. 미리 만등러진 JDBC 관련된 클래스를 사용 할수 있다.
			// 1. Connection : 인터페이스
			// 2. DriverManager : 클래스
	...
	/*	// 박진영의 주소 변경
		...
		*/
		...
		ResultSet rs = ps.executeQuery(); // 쿼리 [ 테이블 조작된 결과 ]
				...
				// 반복문 이용한 레코드 하나씩 출력
					//null ->  검색레코드1 -> 검색레코드2
					//.next() : 다음레코드 호출 [ 다음레코드가 있으면 true 없으면 false 반환]
					//.getString(필드순서번호): 해당필드의 데이터를 문자열로 가져오기
		...
		while (rs.next() ) { // while (조건) : 조건이 true 이면 반복문 실행
			...
			System.out.print("회원명 :"+rs.getString(2)+"\t"); // 두번쨰 필드의 데이터 가져오기
