...
function updatemodal( pno ){
	...
	// 2. 해당 모달에 기존 내용물 넣기 
		...
		$.ajax({
			...
			type : "get" ,
			success : function( re ){ 
				let 제품 = JSON.parse(re)
				document.querySelector('.pno').value = 제품.pno
				document.querySelector('.pname').value = 제품.pname
				document.querySelector('.pcomment').value = 제품.pcomment
				document.querySelector('.pprice').value = 제품.pprice
				document.querySelector('.pdiscount').value = 제품.pdiscount
						...
						$.ajax({
							...
							type : "get" , 
							success:function(re){
								let 카테고리 = JSON.parse(re)
								let html = ''
								
								for( let i = 0 ; i<카테고리.length ; i++ ){
									let category = 카테고리[i];
									
									if( category.pcno == 제품.pcno ){ // 카테고리번호와 현재 내가 수정할 제품ㄴ의 카테고리와 같으면
						...
						// 내가 수정할 제품의 상태와 해당 숫자가 같으면 해당 상태에 체크 저장
						if( 제품.pactive == 0 ){ pactivebtns[0].checked = true }
						if( 제품.pactive == 1 ){ pactivebtns[1].checked = true }
						if( 제품.pactive == 2 ){ pactivebtns[2].checked = true }
			}
		})
}
// 2-2. 수정 처리 메소드 
