...
public class Ex4_자판기 { // class s
	...
	public static void main(String[] args) { //main s
		...
		while(true) { //2. 무한루프 [무한반복]
		...
		else if( 메뉴선택 == 4 ) { //4 를입력했을때
			System.out.println("-----------장바구니------"); //선택 된 제품의 목록
			System.out.println("제품명\t구매수\t가격");
			if( 콜라구매수 >0) {System.out.println("콜라\t"+콜라구매수+"\t"+(콜라구매수*300));}
			if( 콜라구매수 >0) {System.out.println("환타\t"+환타구매수+"\t"+(환타구매수*400));}
			if( 콜라구매수 >0) {System.out.println("사이다\t"+사이다구매수+"\t"+(사이다구매수*500));}
			...
			if(결제성공 == false) {//결제성공 : 장바구니 0 으로 초기화
				콜라구매수 = 0; 환타구매수 = 0 ; 사이다구매수 = 0;
			}else {// 결제취소 : 금액이 부족 하거나 . 결제여부가 N 이면 => 재고를 구매수만큼 다시 채우
...
/* 
 	...
 	자판기 프로그램
 		...
 		3. 메뉴판에서 선택한 제품을 장바구니 담기
 			1. 선택 제품의 재고가 0이면 작으면 재고 없음 출력 -> 나감
 			2. 제품의 수를 1차감 -> 장바구니 제품의 1증가
 			3. '장바구니에 담았습니다'
 		4. 결제를 선택했을때 장바구니 현황 표시 결제
 		5. 결제예정인 금액보다 투입금액 이 부족 결제취소  투입금액이 초과 잔돈 출력
 		[화면구현]
