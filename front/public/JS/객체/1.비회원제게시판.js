/*
	...
	// 4. 게시물 여러개 저장할 공간 = 배열 사용
	// 5. 글 등록이 완료 될때마다 board객체를 배열에 저장한다.
		//6.글 등록했으면 기존 내용물 지우기 -> input 공백넣기
	[출력]
	//1. 새로운 글이 등록 될때마다 [ 글 출력]
	[보기]
	// 1. 게시물 출력해서 제목을 클릭 할때마다 [ 게시물보기]
		// 1. 내가 클릭한 게시물(???) : 매개변수 [클릭한 게시물의 번호 (중복 없는 속성)]
*/
...
function 게시물출력(){
	...
	for( let i = 0 ; i<boardlist.length ; i++){
		// i는 0부터 배열의길이 까지 1씩 증가  				//배열내 i번째 객체내 '제목' 키 호출 -> 값이나옴		
		html += '<tr>'+
							'<td> '+i+' </td> '+
							'<td><a href="#" onclick="게시물보기('+i+')">'+boardlist[i].제목+'</a></td>'+
							'<td>'+boardlist[i].작성자+'</td>'+
					'/<tr>'								// " " :문자 처리 ' ':문자처리 vs `${ }`
	}													// js에서 html 작성지 문자처리 : '<태그명 onclick="함수()">'
	document.getElementById('boardlist').innerHTML = html
} // 게시물 출력함수 end	
function 게시물보기( i){
	document.getElementById('viewtitle').innerHTML = boardlist[i].제목
	document.getElementById('viewtitle').innerHTML = boardlist[i].작성자
	document.getElementById('viewtitle').innerHTML = boardlist[i].내용
} // 게시물보기 함수 end
