...
public class BoardPage {
	...
	void view() {
		...
		while(true) { // 무한루프
			...
			// 경우의수 2개 +1 (그외)
			if( btn ==1) { page.regist();}					//1. 입력하면 등록함수 호출
			else if( btn ==2) {page.getBoard();}			//2. 입력하면 개별게시물 출력화면 함수호출
	...
	void getBoardlist() {
		...
		BoardDto[] boardlist = control.getBoardlist(); // control에서 모든 게시물호출 메소드 호출 -> 모든 게시물의 배열을 반환 받는다
		
		//for( int i = 0; i<boardlist.length; i++) {}
		for (BoardDto dto : boardlist) { // 모든 게시물의 배열을 반복문 돌린다.
		// for(배열의자료형 반복변수명:배열 )
			System.out.print( dto.b_no +"\t");		System.out.print( dto.b_writer +"\t");
			System.out.print(dto.b_title+"\t");	System.out.println(dto.b_view+"\n");
		}
	}// m end
	
	//3.개별게시물 출력 화면
	void getBoard() {
		System.out.println(">> 게시물번호 :"); int b_no = scanner.nextInt(); //보고싶은 게시물 번호 입력받는다.
