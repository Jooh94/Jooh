...
create table board(
    ...
    b_no int primary key auto_increment ,
      -- primary key : 기본키 [ 식별자 : 주민등록번호,사번,학번 등] :중복 없다 
      -- auto_increment : 자동번호 부여 [db에서 레코드 생성시 자동으로 번호가 순서대로 생성 [ 1~] ]
      b_title varchar(100) ,	-- 해당 필드에 최대 100글자 저장가능
      b_content varchar(500), 	-- 해당 필드에 최대 500글자 저장가능
      b_writer varchar(10) ,	-- 해당 필드에 최대 10글자  저장가능
      b_password varchar(4) ,	-- 해당 필드에 최대 4글자	저장가능
      b_view int				-- 해당 필드에 최대 int형[+-20억정도]
      -- int		: 4바이트 정수 +-20억
...
-- 5. 테이블에 레코드 추가
    ...
    -- 	intsert into 테이블명 (필드명1 ,필드명2 ,) values (값1 ,값2)
insert 
	into
...
-- 8.특정[조건] 게시물 조회
    ...
    select * from board where b_writer = '유재석' -- 작성자가 '유재석'인 게시물조회
