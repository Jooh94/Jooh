...
//print , scanner: 입/출력 작성할예정

import java.util.Scanner;

public class view {
	// 함수밖에 scanner 선언한이유 : 모든 함수에서 같이 사용하려고
static Scanner scanner= new Scanner(System.in);
	...
	//1.프로그램 시작하는 화면 구현 함수 [매개변수x 변환x 정적메소드:호출시 객체 생략 가능]
	static void start() {
		while(true) {
	...
	static void view_board( ) {
		...
		else if( ch==3) {view_delete(bnum);} //삭제 화면 함수 호출 [매개변수 1개 int =현재 보고있느 게시물]
		else { }
		return;
	}
